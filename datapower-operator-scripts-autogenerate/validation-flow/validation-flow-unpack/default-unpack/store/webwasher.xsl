...
<xsl:stylesheet
  ...
  <dp:summary xmlns="">
    ...
    <descriptionId>store.webwasher.dpsummary.description</descriptionId>
  </dp:summary>
  ...
  <dp:param name="dpconfig:RemoteAddress" type="dmHostname" xmlns="">
    ...
    <displayId>store.webwasher.param.RemoteAddress.display</displayId>
    <description>Please enter the remote address of the Virus Scanner.</description>
    <descriptionId>store.webwasher.param.RemoteAddress.description</descriptionId>
  </dp:param>
  ...
  <dp:param name="dpconfig:RemotePort" type="dmIPPort" xmlns="">
     ...
     <displayId>store.webwasher.param.RemotePort.display</displayId>
     <description>Please enter the remote port of the Virus Scanner.</description>
     <descriptionId>store.webwasher.param.RemotePort.description</descriptionId>
  </dp:param>
  ...
  <dp:param name="dpconfig:LogCategory" type="dmReference" reftype="LogLabel" xmlns="">
    ...
    <displayId>store.webwasher.param.LogCategory.display</displayId>
    <description>Please select the log category for Virus Scanner logs.</description>
    <descriptionId>store.webwasher.param.LogCategory.description</descriptionId>
  </dp:param>
  ...
  <xsl:template match="/*">
    ...
    <xsl:for-each select="$attachments/manifest/attachments/attachment">
      ...
      <xsl:variable
        name="basicvalue"
        select="concat('req-hdr=0,res-hdr=26,res-body=73',$eol,$eol,'GET http://ww HTTP/1.0',$eol,$eol,'HTTP/1.0 200 OK',$eol,'Transfer-Encoding: chunked')"/>
      ...
      <xsl:variable name="attachment">
        <dp:url-open target="{concat('icap://',$dpconfig:RemoteAddress,':',$dpconfig:RemotePort,'/wwrespmod')}" response="responsecode-ignore" data-type="base64" http-headers="$httpHeaders">
