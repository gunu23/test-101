...
try {
  ...
  var sign = crypto.createSign(constants.signingAlg, undefined, 20);
  sign.update(constants.msgToSign).sign(constants.signingKey, function(error, signature) {
    if (error) {
      throw new Error(error);
    } else {
      signature = signature.toString('base64');
      var verify = crypto.createVerify(constants.signingAlg, undefined, 32);
  ...
  console.error("example-pss-sign-verify-neg.js error: " + e);
  session.output.write("example-pss-sign-verify-neg.js error: " + e);
}