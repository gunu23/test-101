...
(C) Copyright IBM Corporation 2007, 2020
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- given a fragment of defaults, select the first licensed one. -->
  <func:function name="dpfunc:get-licensed-default">
    <xsl:param name="propertyOrType"/>
    <xsl:variable name="def">
      <xsl:choose>
        <xsl:when test="$propertyOrType/default">
          <xsl:value-of select="$propertyOrType/default[dpfunc:IS-LICENSED(@licensed)]"/> 
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$propertyOrType//value[@default = 'true' and dpfunc:IS-LICENSED(@licensed)]"/> 
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <func:result select="$def"/>    
  </func:function>
          ...
          <xsl:element name="{@name}">
            ...
            <xsl:choose>
              ...
              <xsl:when test="default">
                <xsl:value-of select="dpfunc:get-licensed-default(.)"/>
              </xsl:when>
              ...
              <xsl:when test="$typeNode/type/default">
                ...
                <xsl:value-of select="dpfunc:get-licensed-default($typeNode/type)"/>
              </xsl:when>
            </xsl:choose>
          </xsl:element>
        </xsl:if>
