...
<!-- 
  ...
  This stylesheet is to be used in conjunction with a configured XML firewall
  as a client for receiving policy updates from TSPM 7.1.0.1
...
<xsl:stylesheet xmlns:tspm="http://security.tivoli.ibm.com/policy/distribution" 
    ...
    <xsl:variable name="wssecLoopbackPort">2048</xsl:variable>
    ...
    <xsl:variable name="policyAttachmentElementName">wsp:PolicyAttachment</xsl:variable>
    <xsl:variable name="policyElementName">wsp:Policy</xsl:variable>
    ...
    <!-- 
        Handle each PolicyDistributionUpdate message in the incoming
        WS-Notification message.
        
        The PolicyDistributionUpdate message contains both the location to get
        the update (PolicySource) and the protocol to do it (Protocol).
    ...
    <xsl:template name="handle-commit">
        ...
        <xsl:choose>
                ...
                <xsl:variable name="rst">
                    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" 
                        ...
                        <soapenv:Header>
                            ...
                            <!-- Copy all the reference parameters to our
                                 output SOAP header, as per the WS-Addressing
                                 protocol
                            -->
         ...
         <xsl:variable name="wssecLoopbackAddr" select="concat( 'http://localhost:', $wssecLoopbackPort, '/com.ibm.tspm.distribution.policy/PolicyService' )" />
                <xsl:variable name="result" select="dp:soap-call($wssecLoopbackAddr, $rst, '', 0, $wsmex-action)"/>
    ...
    <xsl:template name="handle-remove">
        ...
        <xsl:if test="$dialect-directory != ''">
            ...
            <!-- 
                The DP administration message for removing the policy for for
                this context
            -->
    ...
    <!--
        This section handles processing each individual <mex:MetadataSection>
        in the response from the Policy Source.

        Each section contains some policy, which must be written to disk. This
        stylesheet calls back to the DataPower SOAP management interface to
        write the files.
    ...
    <xsl:template match="mex:MetadataSection">
        ...
        <xsl:if test="$dialect-directory != ''">
            <xsl:message dp:priority="debug">Dialect directory is <xsl:value-of select="$dialect-directory"/></xsl:message>
            <!--
                For message protection policies, there are multiple
                MetdataSection entries so calculated a suffix based on
                relevant attributes or elements
            -->
            <xsl:variable name="suffix">
                <xsl:for-each select="child::*">
                    <xsl:choose>
                        <!--  If it is a policy attachment, use the AppliesToi
                              URI but clean it so it is not as long -->
                        <xsl:when test="name() = $policyAttachmentElementName">
                            <xsl:message dp:priority="debug">policySet is <dp:serialize select="node()"/></xsl:message>
                            <xsl:variable name="attachmentUri" select="wsp:AppliesTo/wsp:URI/text()"/>
                            <xsl:message dp:priority="debug">Attachment URI is: <xsl:value-of select="$attachmentUri"/></xsl:message>
                            <xsl:variable name="cleanattachmentUri" select="regExp:replace( $attachmentUri, '[A-Z0-9._%-:#]+\(', 'gi', '' )"/>
                            <xsl:message dp:priority="debug">Cleaned attachment URI is: <xsl:value-of select="$cleanattachmentUri"/></xsl:message>
                            <xsl:variable name="removedAttachmentInvalidChars" select="regExp:replace($cleanattachmentUri , '[:|/|\)]', 'gi', '_' )"/>
                            <xsl:message dp:priority="debug">Storable attachment URI is: <xsl:value-of select="$removedAttachmentInvalidChars"/></xsl:message>
                            <xsl:value-of select="concat('_', $removedAttachmentInvalidChars, 'attach')" />
                        </xsl:when>
                        <xsl:when test="name() = $policyElementName">
                            <xsl:variable name="policyName" select="./@Name"/>
                            <xsl:variable name="cleanedPolicyName" select="regExp:replace($policyName , '[:|/]', 'gi', '_' )" />
                            <xsl:value-of select="concat('_', $cleanedPolicyName)"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="filename" select="concat( 'local:///tspm/', $dialect-directory, '/', $encoded-context, $suffix, '.xml' )"/>
            ...
            <!--
                The DP administration message for creating the directory to
                put files in.
            ...
            <!-- 
               The DP administration message for uploading our newly received
               policy file. Any existing policy for this context + dialect will
               be OVERWRITTEN!
            -->
            ...
            <!-- 
                The DP administration message for notifying DataPower that the
    ...
    <xsl:template name="do-admin-request">
        ...
        <xsl:message dp:priority="debug">Making DP administrative requests to <xsl:value-of select="$redacted_url"/></xsl:message>
