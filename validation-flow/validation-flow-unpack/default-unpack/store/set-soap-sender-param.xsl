...
<!--
    Provide common WS-Sec related SOAP sender parameters and libs.
-->
...
>
  ...
  <xsl:key name="soap-actor-role" 
           match="/*[local-name()='Envelope']/*[local-name()='Header']/*[local-name()='Security']
                        [dpfunc:match-actor-role(., $resolved-actor-role-id, 'sender')]" 
  ...
  <!-- If there is an existing Security element then use the first one.
       ...
       We can not modify our existing codes to loop all the applicable Security headers, customer
       should configure multiple actions to process the implicit actor/roles.
       -->
